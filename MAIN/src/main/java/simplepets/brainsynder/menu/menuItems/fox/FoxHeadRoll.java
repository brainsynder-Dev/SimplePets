...
public class FoxHeadRoll extends MenuItemAbstract<IEntityFoxPet> {
    ...
    public FoxHeadRoll(PetType type, IEntityPet entityPet) {
        super(type, entityPet);
    }
    public FoxHeadRoll(PetType type) {
        super(type);
    }
    ...
    public ItemBuilder getItem() {
        ...
        builder.withName(formatName(builder, (entity, name) -> {
            name = name.replace("%value%", Messages.getTrueOrFalse(entity.isRollingHead()));
            return name;
        }));
        return builder;
    }
