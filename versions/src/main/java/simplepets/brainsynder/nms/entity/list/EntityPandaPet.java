...
public class EntityPandaPet extends EntityAgeablePet implements IEntityPandaPet {
    ...
    private void handleSneeze() {
        ...
        double x = (this.getX() - (double)(this.getBbWidth() + 1.0F) * 0.5D * (double)(float) Math.sin(this.yBodyRot * 0.017453292F));
        double y = (this.getY() + this.getEyeY() - 0.10000000149011612D);
        double z = (this.getZ() + (double)(this.getBbHeight() + 1.0F) * 0.5D * (double)(float) Math.cos(this.yBodyRot * 0.017453292F));
        level().addParticle(ParticleTypes.SNEEZE, x, y, z, var0.x, 0.0D, var0.z);
        this.playSound(SoundEvents.PANDA_SNEEZE, 1.0F, 1.0F);

        List<EntityPandaPet> nearby = level().getEntitiesOfClass(EntityPandaPet.class, this.getBoundingBox().inflate(10.0D));
